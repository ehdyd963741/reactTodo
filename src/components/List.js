...
const List = React.memo(({ id, title, completed, todoData, setTodoData }) => {
  // 편집중인 상태 체크 isEditing? 초기값은 false
  const [isEditing, setIsEditing] = useState(false);
  const [editedTitle, setEditedTitle] = useState(title);
  //  react 에서 css 를 {} 생성하는법
  // btnStyle은 다른 외부 프로세스를 사용했으므로 필요없지만 공부를 위해 남겨둠
  // const btnStyle = {
  //   color: "skyblue",
  //   border: "none",
  //   padding: "5px 9px",
  ...
  if (isEditing) {
    // 편집 목록 출력 상태;
    return (
      <div className="flex items-center justify-between w-full px-4 py-1 my-1 text-gray-600 border rounded">
